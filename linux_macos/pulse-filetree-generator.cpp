...
void listdir(const char *name, FILE * fp,std::set<std::string> &parts )
{
    DIR *dir;
    struct dirent *entry;
    char espacea[5] = " {";
...
void usage(int argc, char* argv[]){
    ...
    cout << "\t\t -f \"file configuration\"   /etc/pulse-xmpp-agent/agentconf.ini par default" << endl;
    cout << "\t\t -o \"file output\"          /tmp/file.txt par default" << endl;
    cout << endl << "\tExample :" << endl;
    cout << "\t" << argv[0] << " -r \"/\" -o \"/tmp/azerty.txt\" -f \"etc/pulse-xmpp-agent/agentconf.ini\""<<endl;
    cout << endl ;
    cout << "\t definir listexclude dans fichier de configuration section browserfile" << endl;
    cout << "example :" << endl;
    cout << "[browserfile]" << endl;
    cout << "defaultdir = /tmp" << endl;
    cout << "rootfilesystem = /tmp" << endl;
    cout << "listexclude = /usr, /lib,/boot,/cdrom,/dev,/lib32,/lib64,/lost+found,/media,/mnt,/opt,/partage,/proc,/root,/run,/sbin,/snap,/somewhere,/srv,/sys" << endl;
}
//________________________________________________________________
//________________________________________________________________
//________________________________________________________________
...
int main(int argc, char* argv[]) {
    ...
    char * rootfilesystem = getCmdOption(argv, argv + argc, "-r");
    if (!rootfilesystem)
    {
        cerr << "Error : root file system missing option -r \"rootfilesystem\"" << endl;
        usage(argc, argv);
        return -1;
    }

    if (!dir_exist(rootfilesystem)){
        cerr << "Error : folder "<< rootfilesystem << " does not exist" << endl;
    ...
    if (file_exist(filenameconf)){
        config cfg(filenameconf);
        section* usersection = cfg.get_section("browserfile");
        if (usersection != NULL) {
...
//                 std::cout << "rootfilesystem=" << cfg.get_value("browserfile", "rootfilesystem") << '\n';
                std::cout << "exclusion list is " << cfg.get_value("browserfile", "listexclude") << '\n';
            }
